...
def is_off(slice):
    # if cell is off and surrounding neighbors are off
    if slice[0] == 0 and slice[1] == 0 and slice[2] == 0:
        return True

    # if cell is on and surrounding neighbors are on
    if slice[0] == 1 and slice[1] == 1 and slice[2] == 1:
        return True
...
def get_new_value(old_gen, old_automata):
    ...
    row_start = old_gen * 50

    previous_row = old_automata[row_start:row_start + 49]
    current_row = old_automata[row_start + 50:row_start + 99]
    ...
    while i < len(current_row) - 1:
        # grab previous row cell plus neighbors to determine if corresponding cell in current row is on/off
        slice = [previous_row[i - 1], previous_row[i], previous_row[i + 1]]
    ...
    # replace the current row with updated cells
    new_automata[row_start + 50:row_start + 99] = current_row
...
dec_time_step_button = pygame.draw.rect(screen, (175, 203, 255),
                                        pygame.Rect(20 + 3 * BTN_SIZE, WIN_SIZE + 10, 3 * BTN_SIZE, BTN_SIZE))
stop_play_button = pygame.draw.rect(screen, (175, 203, 255),
                                    pygame.Rect(30 + 6 * BTN_SIZE, WIN_SIZE + 10, 3 * BTN_SIZE, BTN_SIZE))
restart_button = pygame.draw.rect(screen, (175, 203, 255),
                                  pygame.Rect(40 + 9 * BTN_SIZE, WIN_SIZE + 10, 3 * BTN_SIZE, BTN_SIZE))
generation_display = pygame.draw.rect(screen, GRAY,
                                      pygame.Rect(60 + 12 * BTN_SIZE, WIN_SIZE + 10, 3 * BTN_SIZE, BTN_SIZE))
...
while not done:
    ...
    if running:
        ...
        inc_time_step_button = pygame.draw.rect(screen, (175, 203, 255),
                                                pygame.Rect(10, WIN_SIZE + 10, 3 * BTN_SIZE, BTN_SIZE))
